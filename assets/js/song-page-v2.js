...
let song=null,parts=[],primaryPart=null,selectedSection=-1,cursor=0,flat=[];
let playing=false,loop=false,audio=null,timers=[],nodes=[],runId=0;
...
function normalizeParts(){
  const raw=Array.isArray(song.parts)&&song.parts.length?song.parts:[{
    id:"melody",name:"Melody",role:"lead",defaultInstrument:RECOMMENDED[slug]||song.playback?.defaultInstrument||"guitar",sections:song.sections
  }];
  parts=raw.map((p,i)=>({
    id:p.id||`part-${i+1}`,
...
function renderHero(){
  ...
  const badges=[song.arrangement.tuning.name,song.arrangement.difficulty.label,song.musical.meter,`max fret ${song.stats.maxFret}`,`timing ${song.verification.timing.confidence}`,`${parts.length} part${parts.length===1?"":"s"}`,"MIDI export"];
...
function renderControls(){
  $("#tempo").min=song.musical.bpmRange[0];$("#tempo").max=song.musical.bpmRange[1];$("#tempo").value=song.musical.defaultBpm;$("#tempoOut").textContent=song.musical.defaultBpm;
  $("#gate").value=100;$("#gateOut").textContent="100%";
  const st=partState.get(primaryPart.id);$("#instrument").innerHTML=instrumentOptions(st.instrument);$("#instrument").value=st.instrument;
}
...
function renderTabs(){
  ...
  activeSectionsFor(primaryPart).forEach(section=>{
    ...
    (section.measures||[]).forEach((m,mi)=>{
      ...
      (m.events||[]).forEach((e,ei)=>{if(e.type!=="note")return;const local=m.duration?(.055*m.duration+Number(e.start||0)*.89):Number(e.start||0);const x=((measureStart+local)/total)*100;
        const n=document.createElement("button");n.type="button";n.className=`note ${e.position}${e.anchor?" anchor":""}`;n.textContent=e.fret;n.style.left=`${x}%`;n.style.top=`${STRING_Y[e.string]}px`;n.dataset.section=section.id;n.dataset.measure=mi;n.dataset.event=ei;n.title=`${e.string} string · fret ${e.fret} · ${e.duration} timing units`;n.onclick=()=>audition(section.id,mi,ei,n);staff.appendChild(n);
      });
...
function renderOrchestra(){
  ...
  const rows=parts.map(p=>{
    ...
    const sectionRows=p.sections.map((s,i)=>{const key=`${p.id}:${s.id}`,inst=sectionInstrument.get(key)||"";return `<div class="section-orch-row"><span>${safe(s.name)}</span><select data-section-inst="${safe(key)}"><option value="">Use part instrument</option>${instrumentOptions(inst)}</select></div>`;}).join("");
    return `<div class="part-row" data-part="${safe(p.id)}"><label class="part-enable"><input type="checkbox" data-part-enabled="${safe(p.id)}" ${st.enabled?"checked":""}><b>${safe(p.name)}</b><small>${safe(p.role)}</small></label><select data-part-inst="${safe(p.id)}">${instrumentOptions(st.instrument)}</select><details><summary>Section overrides</summary>${sectionRows}</details></div>`;
  }).join("");
  host.innerHTML=`<div class="orchestra-head"><div><div class="eyebrow">Mini orchestra</div><h2>Orchestration</h2><p>Each simultaneous part gets its own instrument; each section can override it.</p></div><button id="playAllParts" class="primary">▶ Play full arrangement</button></div><div class="orchestra-grid">${rows}</div><div class="orchestra-note">Current songs may contain only one transcribed part. The player is now multi-part capable; new songs can add real concurrent violin, flute, cello, bass, etc. tracks without changing the UI.</div>`;
  ...
  $("#playAllParts").onclick=()=>{selectedSection=-1;cursor=0;renderSectionButtons();renderTabs();buildFlat();highlight();play();};
}
...
function clearTimers(){timers.forEach(clearTimeout);timers=[];}function later(fn,ms){const id=setTimeout(fn,Math.max(0,ms));timers.push(id);}function stopNodes(){nodes.forEach(n=>{try{n.stop();}catch(_){}});nodes=[];}
function stop(reset=true){runId++;playing=false;clearTimers();stopNodes();if(reset)cursor=0;document.querySelectorAll(".note.live,.note.next").forEach(n=>n.classList.remove("live","next"));updateButtons();highlight();updateStatus();}
...
function allPlaybackEvents(){
  const salvage=$("#salvage").checked;
  ...
  return parts.filter(p=>partState.get(p.id)?.enabled).flatMap(p=>eventsForPart(p).map(x=>({...x,instrument:currentInstrument(p,x.section)}));
}
function play(){
  if(!flat.length)return;ensureAudio();stopNodes();clearTimers();playing=true;updateButtons();const myRun=++runId,spu=secondsPerUnit(),gate=Number($("#gate").value)/100;
  const currentStart=flat[cursor]?.start??0,events=allPlaybackEvents().filter(x=>x.start>=currentStart-1e-6),audioStart=audio.currentTime+.08;
  events.forEach(x=>{const rel=(x.start-currentStart)*spu;synthTone(x.event.midi,audioStart+rel,Math.max(.06,Number(x.event.duration||.25)*spu*gate),x.instrument,.9);});
  const primaryEvents=flat.map((x,i)=>({...x,flatIndex:i})).filter(x=>x.start>=currentStart-1e-6);
  primaryEvents.forEach(x=>{const rel=(x.start-currentStart)*spu;later(()=>{if(myRun!==runId)return;cursor=x.flatIndex;highlight();updateStatus();},80+rel*1000);});
  const last=events.reduce((a,b)=>(!a||b.start+Number(b.event.duration||0)>a.start+Number(a.event.duration||0))?b:a,null);const total=last?((last.start-currentStart)+Number(last.event.duration||0))*spu*1000:0;
  later(()=>{if(myRun!==runId)return;if(loop){cursor=0;play();}else{playing=false;updateButtons();updateStatus();}},total+160);
}
...
function updateStatus(){const total=flat.length,pos=total?cursor+1:0;$("#counter").textContent=`${pos} / ${total}`;$("#progress").style.width=total?`${(pos/total)*100}%`:"0%";$("#status").textContent=$("#salvage")?.checked?`Salvage mode · ${total} anchors`:(playing?"Playing":(cursor?"Paused / positioned":"Ready"));}
...
function vlq(n){let b=[n&127];while(n>>=7)b.unshift((n&127)|128);return b;}function u32(n){return[(n>>>24)&255,(n>>>16)&255,(n>>>8)&255,n&255]}function u16(n){return[(n>>>8)&255,n&255]}
function exportMidi(){
  ...
  const enabled=parts.filter(p=>partState.get(p.id)?.enabled).slice(0,15);enabled.forEach((p,pi)=>{const ch=pi>=9?pi+1:pi;let lastInst=null,sectionBase=0;activeSectionsFor(p).forEach(section=>{const inst=currentInstrument(p,section),prog=INSTRUMENTS[inst]?.program??0;if(prog!==lastInst){timeline.push({t:Math.round(sectionBase*ticksPerUnit),kind:"program",ch,prog});lastInst=prog;}(section.measures||[]).forEach((m)=>{(m.events||[]).forEach(e=>{if(e.type!=="note")return;const on=sectionBase+Number(e.start||0),off=on+Number(e.duration||.25);timeline.push({t:Math.round(on*ticksPerUnit),kind:"on",ch,p:e.midi});timeline.push({t:Math.round(off*ticksPerUnit),kind:"off",ch,p:e.midi});});sectionBase+=Number(m.duration||0);});});});
...
function bind(){
  ...
  document.addEventListener("keydown",e=>{if(["INPUT","SELECT","TEXTAREA"].includes(e.target.tagName))return;if(e.code==="Space"){e.preventDefault();playing?pause():play();}if(e.code==="ArrowRight"){e.preventDefault();step(1);}if(e.code==="ArrowLeft"){e.preventDefault();step(-1);}});
}
